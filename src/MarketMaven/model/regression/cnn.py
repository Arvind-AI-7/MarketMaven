...
class LSTMWithDropout(nn.Module):
    def __init__(self, input_size, hidden_size, bidirectional=True, dropout_prob=0.5):
...
class Model_LSTM_CNN(nn.Module):
    def __init__(self, train_x, test_x, train_y, test_y, X_forecast):
        super(Model_LSTM_CNN, self).__init__()
        ...
        #     LSTMWithDropout(32, 50, bidirectional=False, dropout_prob=0.2),
        # )
    ...
    def forward(self, x):
        ...
        x = self.final_layer(x)  # Take the last output of the LSTM sequence
        return x
    ...
    def model(self):
        ...
        for epoch in range(num_epochs):
            ...
            print(f"CNN Epoch {epoch + 1}/{num_epochs}, Loss: {avg_loss}")
