...
class Model_LSTM_GRU(nn.Module):
    def __init__(self, train_x, test_x, train_y, test_y, X_forecast):
        super(Model_LSTM_GRU, self).__init__()
        ...
        self.lstm1 = nn.LSTM(input_size=self.train_x.shape[1], hidden_size=8, num_layers=1, batch_first=True)
        self.lstm2 = nn.LSTM(input_size=8, hidden_size=8, num_layers=1, batch_first=True)
        self.gru1 = nn.GRU(input_size=8, hidden_size=8, num_layers=1, batch_first=True)
        self.gru2 = nn.GRU(input_size=8, hidden_size=8, num_layers=1, batch_first=True)
        self.gru3 = nn.GRU(input_size=8, hidden_size=8, num_layers=1, batch_first=True)
    ...
    def model(self):
        ...
        for epoch in range(num_epochs):
            ...
            print(f"LSTM_GRU_RNN Epoch {epoch+1}/{num_epochs}, Loss: {avg_loss}")
